...
public class Busqueda extends JFrame {
	 ...
	 */
	public Busqueda() {
		System.out.println("Busqueda ejecutada");
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					if(confirmarh == JOptionPane.YES_OPTION){
						...
						huespedesController.eliminar(Long.valueOf(valor));
						JOptionPane.showMessageDialog(contentPane, "Registro Eliminado");
						limpiarTabla();
						LlenarTablaHuespedes();
						LlenarTablaReservas();
	...
	private void LlenarTablaReservas() {
		System.out.println("LlenarTablaReservas Ejecutada");
		List<Reserva> reserva = BuscarReservas();
		try {
			for (Reserva reservas : reserva) {
				modelo.addRow(new Object[] { reservas.getNumeroReserva(), reservas.getCheckIn(), reservas.getCheckOut(), reservas.getValor(), reservas.getFormaDePago() });
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void LlenarTablaReservasId() {
		System.out.println("LlenarTablaReservasId Ejecutada");
	...
	private void ActualizarReservas() {
		System.out.println("Actualizar Reservas Ejecutada");
		...
		.ifPresentOrElse(fila -> {
			...
			this.reservaController.actualizar(checkIn,checkOut, valor, formaDePago, id);
	...
	private void ActualizarHuesped() {
		System.out.println("Actualizar Huesped Ejecutada");
